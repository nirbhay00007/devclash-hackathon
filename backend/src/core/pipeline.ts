...
export function _setLastGlobalSummary(summary: GlobalRepoSummary) { _lastGlobalSummary = summary; }
...
export async function runIngestionPipeline(
    ...
    try {
        ...
        await Promise.all(rawNodes.map(node => limit(async () => {
            ...
            // Get current graph state in React Flow format for live building effect
            const { nodes, edges } = globalGraph.exportForReactFlow(repoId);
        ...
        try {
            ...
            _lastGlobalSummary = {
                overallPurpose: 'Architectural summary currently unavailable (Gemini phase skipped or failed).',
        ...
        const finalGraph = globalGraph.exportForReactFlow(repoId);
        ...
        sendSSE(sseRes, {
            ...
            progress: 100,
            language: detectedLanguage,
            globalSummary: _lastGlobalSummary,
