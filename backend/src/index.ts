...
import { runIngestionPipeline, getLastGlobalSummary, _setLastGlobalSummary, isPipelineRunning, isRepoRunning, PipelineOptions } from './core/pipeline';
import { globalGraph } from './storage/graphStore';
import { semanticSearch, initVectorStore } from './storage/vectorStore';
import { askGeminiArchitect, generateGlobalRepoSummary } from './ai/geminiIntelligence';
...
// Force-generates the global summary using the existing in-memory graph.
app.post('/api/summary/generate', async (req, res) => {
    const { apiKey } = req.body;
    try {
        const nodes = globalGraph.getAllNodes();
        if (nodes.length === 0) {
            return res.status(400).json({ success: false, error: 'No graph data found. Please run a repository analysis first.' });
        }

        // Use provided key or fallback to env
        const effectiveKey = apiKey?.trim() || process.env.GEMINI_API_KEY;
        if (!effectiveKey) {
            return res.status(401).json({ success: false, error: 'Missing Gemini API Key. Please provide one in the input field or .env file.' });
        }

        const geminiPayload = nodes.map(n => ({
            path:           n.id,
            summary:        n.summary || '',
            responsibility: n.responsibility || '',
            complexity:     n.complexity || 'low',
            patterns:       n.patterns || [],
            external_deps:  n.externalDeps || [],
            is_entry_point: n.isEntryPoint || false,
            key_exports:    n.keyExports || [],
            layer:          n.layer || 'unknown',
            code_quality:   n.codeQuality || 'acceptable',
        }));

        const summary = await generateGlobalRepoSummary(geminiPayload, effectiveKey);
        _setLastGlobalSummary(summary);

        res.json({ success: true, summary });
    } catch (err: any) {
        console.error('[API /api/summary/generate] ❌ Error:', err);
        res.status(500).json({ 
            success: false, 
            error: err.message || 'Gemini failed to generate summary. Ensure your API Key is valid and has sufficient quota.' 
        });
    }
});
