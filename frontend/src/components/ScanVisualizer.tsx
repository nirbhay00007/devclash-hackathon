...
function CircularProgress({ pct, total, color }: { pct: number; total: number; color: string }) {
  const R_OUTER = 120;
  const R_INNER = 90;
  ...
  const filesScanned = Math.round((pct / 100) * total);
    ...
    <div style={{ position: 'relative', width: 280, height: 280, flexShrink: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2 }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 36, fontWeight: 900, color: 'var(--black)', letterSpacing: '-1px', lineHeight: 1 }}>
          {filesScanned}
          <span style={{ fontSize: 18, color: '#d1d5db', fontWeight: 500 }}>/{total}</span>
        </div>
...
export default function ScanVisualizer({ repo, onStop, termRef }: Props) {
  ...
  // Sidebar nav items
  const sideItems = [
    { icon: '📄', label: 'Files',  count: nodes.length },
    { icon: '⚙️', label: 'Logic',  count: null },
    { icon: '🔗', label: 'Deps',   count: edges.length },
    { icon: '📊', label: 'State',  count: null },
    ...
    <div style={{ display: 'flex', flex: 1, overflow: 'hidden', height: '100%' }}>
      ...
      <div style={{ width: 180, flexShrink: 0, borderRight: '1px solid var(--gray-200)', background: '#fafafa', display: 'flex', flexDirection: 'column', padding: '16px 0' }}>
        ...
        <div style={{ padding: '12px 16px', borderTop: '1px solid var(--gray-200)' }}>
          <button style={{ width: '100%', padding: '7px', borderRadius: '6px', border: '1px solid var(--gray-200)', background: 'var(--white)', fontSize: 10.5, fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.07em', cursor: 'pointer' }}>
            Expand All
          </button>
        </div>
        ...
        <div style={{ padding: '8px 16px', display: 'flex', flexDirection: 'column', gap: 4 }}>
          {['⚙', '👤'].map((icon, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 4px', fontSize: 12.5, color: '#9ca3af', cursor: 'pointer', borderRadius: 6 }}>
              <span>{icon}</span>
              <span style={{ fontWeight: 500 }}>{i === 0 ? 'Settings' : 'User'}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--white)', borderRight: '1px solid var(--gray-200)', minWidth: 0 }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, padding: '20px' }}>
          <CircularProgress pct={progress} total={Math.max(nodes.length, 50)} color={color} />
        </div>
